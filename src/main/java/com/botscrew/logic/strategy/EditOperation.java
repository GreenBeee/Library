...
public class EditOperation implements Strategy {
    ...
    public String doOperation(String input) {
        ...
        try {
            ...
            List<Book> allBooks = service.getBookByName(input.substring(4).trim());

            if (allBooks.size() == 1) {

                allBooks.get(0).setName(getNewBookName());
                service.updateBook(allBooks.get(0));
                answer.append(allBooks.get(0)).append(" was edited");

            } else if (allBooks.size() > 1){

                Book toRemove = chooseBookToDelete(allBooks);
                toRemove.setName(getNewBookName());
                service.updateBook(toRemove);
                answer.append(toRemove).append(" was edited");
